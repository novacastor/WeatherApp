...
import {
    ...
    saveToStorage,
    getFromStorage
} from '../assets/weather-data/storageUtils.js';
...
export const markForPermanentStorage = (city_name = currentCity) => {
    let location = locations.find((loc) => loc.city.toLowerCase() === city_name.toLowerCase());
    if(location) {
...
async function fetchData(city_name = currentCity, to_save = false) {
    ...
    const startIndex = raw.hourly.time.findIndex(time => time === currentHourISO);
    ...
    const newLocationData = {
        ...
        dailyForecast: raw.daily.time.map((date, index) => {
            ...
            return {
                ...
                humidity: raw.hourly.relative_humidity_2m ? raw.hourly.relative_humidity_2m[dayIndex] : raw.current.relative_humidity_2m,
                cloudCover: raw.hourly.cloud_cover ? raw.hourly.cloud_cover[dayIndex] : 0,
            };
        }),
        hourlyForecast: raw.hourly.time.slice(safeStart, safeStart + 7).map((time, index) => {
    ...
    let locationIndex = locations.findIndex((loc) => 
        loc.city.toLowerCase() === city_name.toLowerCase()
    );
    if(locationIndex !== -1) {
...
async function getData(city_name = currentCity, to_save = false) {
    ...
    return locations.find((loc) => loc.city.toLowerCase() === city_name.toLowerCase());
}
